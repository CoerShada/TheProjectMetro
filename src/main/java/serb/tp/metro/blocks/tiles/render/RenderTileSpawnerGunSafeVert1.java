...
public class RenderTileSpawnerGunSafeVert1 extends TileEntitySpecialRenderer {

	private int list;
	ResourceLocation texture =  new ResourceLocation(Main.modid, "textures/models/items/armor/backpack_gekkon.png");
	
    public RenderTileSpawnerGunSafeVert1(ResourceLocation res)
    {
        IModelCustom model;
	...
	public void renderTileEntityAt(TileEntity tileentity, double d0, double d1, double d2, float pticks) {
		...
		if(meta==0) {
			GL11.glTranslated(d0+0.5, d1+1.50, d2+1);
			GL11.glRotated(180, 0, 1, 0);
			
		}
		else if(meta==1) {
			GL11.glTranslated(d0, d1+1.5, d2+0.5);
			GL11.glRotated(90, 0, 1, 0);
		}
		else if(meta==2) {
			GL11.glTranslated(d0+0.5, d1+1.5, d2);
			GL11.glRotated(0, 0, 1, 0);
		}
		else if(meta==3) {
			GL11.glTranslated(d0+1, d1+1.5, d2+0.5);
