...
public class ChangeSpawnProbabilityMessage extends AbstractServerMessage<ChangeSpawnProbabilityMessage>{
	...
	public ChangeSpawnProbabilityMessage() {
	}
	
	public ChangeSpawnProbabilityMessage(int index, float value, int x, int y, int z) {
		this.index = index;
		this.value = value;
		this.x = x;
		this.y = y;
	...
	public void process(EntityPlayer player, Side side) {
		...
		if (server.getEntityWorld().getTileEntity(x, y, z)!=null && server.getEntityWorld().getTileEntity(x, y, z) instanceof TileEntityStorageSpawner) {
			...
			PacketDispatcher.sendToAll(new SyncGuiContainerSpawnerMessage(index, value, x, y, z));
		}
